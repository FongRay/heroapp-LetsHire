class Candidate < ActiveRecord::Base
  ...
  scope :with_interview, lambda { where(:current_opening_candidate_id => OpeningCandidate.joins(:interviews).pluck(:opening_candidate_id).uniq) }
  ...
  scope :without_assessment, lambda { where('current_opening_candidate_id > 0 AND current_opening_candidate_id NOT IN (?)',
                                   OpeningCandidate.joins(:assessment).where('"assessments"."comment" IS NOT NULL').map(&:id).uniq.join(',')).with_interview }
